window.artPlugins = window.artPlugins || function (t) {
    ...
    function h() {
        // 修改：总是返回已赞助状态，移除LeanCloud依赖
        return Promise.resolve({
            toJSON: () => ({
                ON: true,
                authData: { baidu: { uid: "anonymous" } },
                check: 3,
    ...
    function x() {
        ...
        t.on("video:ended", () => {
            h().then(e => {
                ...
                Math.max(Date.parse(o) - Date.now(), 0) || t.layers.update({
                    name: "potser",
        ...
        o.on(e.Events.FRAG_LOADED, d((e, o) => {
            h().then(e => {
                t.emit("user", e.toJSON());
                t.once("user", ({ expire_time: t }) => {
                    Math.max(Date.parse(t) - Date.now(), 0) ? b() : null // 移除赞助弹窗调用
                })
            })
    ...
    const { i18n: e, proxy: o, option: n, controls: a, constructor: { utils: { query: s, isMobile: r } } } = t,
        i = { showtext: !r, icon: '<i class="art-icon"><svg class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="22" height="22"><path d="M810.666667 384H85.333333v85.333333h725.333334V384z m0-170.666667H85.333333v85.333334h725.333334v-85.333334zM85.333333 640h554.666667v-85.333333H85.333333v85.333333z m640-85.333333v256l213.333334-128-213.333334-128z" fill="#ffffff"></path></svg></i>' };
        ...
        t.once("user", ({ expire_time: t }) => {
            Math.max(Date.parse(t) - Date.now(), 0) && function(t = []) {
                t.length <= 1 && a.cache.get("PlayList") && a.remove("playlist") : a.update({
                    ...
                    selector: t.map((t, e) => ({ ...t, html: t.name, style: { textAlign: "left" } })),
                    ...
                    mounted: () => {
                        const t = a.cache.get("playlist"),
                            ...
                            r = s(".art-selector-list", e),
                            i = s(".art-selector-value", e),
                            l = r.offsetHeight,
    ...
    return e.update({ "zh-cn": { PlayList: "播放列表" } }), t.isReady ? l() : t.once("ready", l), {
        name: "playlist"
    }
}, () => t => {
    ...
    return e.update({ "zh-cn": { Custom: "自定义" } }), c.unshift(0), i.update({
        ...
        icon: o.playbackRate,
        selector: c.map(t => ({ value: t, name: `playback-rate-${t}`, default: t === b(), html: f(t) })),
        onSelect(e) {
            if (e.value) t.playbackRate = e.value, h(g(), "display", "none");
            else {
                // 修改：移除赞助验证，直接显示自定义速度输入框
    ...
    function x() {
        const { getUrl: t, adToken: e } = o,
            n = t("M3U8_SUBTITLE_SRT") + "&adToken=" + encodeURIComponent(e);
        return fetch(n).then(t => t.ok ? t.text() : Promise.reject()).then(t => {
            ...
            return Promise.all(e.map(t => (function(t, e) {
                return fetch(t, {
                    headers: {
                        range: "bytes=".concat(Array.isArray(e) ? e.join("-") : e || "0-"),
                        referer: location.protocol + "//" + location.host + "/",
                        "User-Agent": "pan.baidu.com"
                    }
                }).then(t => t.ok ? t.blob() : Promise.reject())
            })(t.url).then(e => b(e).then(e => ({
                ...t,
                html: t.name,
                default: "YES" === t.default,
                type: function(t) {
    ...
    function y(t = []) {
        if (!t.length) return;
        const e = t.find(t => t.default) || Object.assign(t[0], { default: !0 }),
            a = Object.assign({}, o.subtitle, { style: o.subtitle.style }, e),
            { url: s, type: r } = a;
    ...
    function w(t = []) {
        i.cache.get("subtitle") ? i.update({ name: "subtitle", selector: t.map((t, e) => ({ ...t })) }) : y(t)
    }
    ...
    return r.update({
        ...
        selector: [{ html: "显示", name: "subtitle", tooltip: "显示", switch: !0, onSwitch: t => (t.tooltip = t.switch ? "隐藏" : "显示", c.show = !t.switch, !t.switch), mounted(e, o) {
            const n = c.show;
        ...
        } }, { html: "字幕偏移", name: "subtitle-offset", tooltip: "0s", range: [0, -10, 10, .1], onChange(e) {
            const o = e.range[0];
        ...
        } }, { html: "字体类型", name: "subtitle-fontFamily", tooltip: e.get("Default"), selector: [{ html: "默认", value: "" }, { html: "等宽 衬线", value: '"Courier New", Courier, "Nimbus Mono L", "Cutive Mono", monospace' }, { html: "比例 衬线", value: '"Times New Roman", Times, Georgia, Cambria, "PT Serif Caption", serif' }, { html: "等宽 无衬线", value: '"Deja Vu Sans Mono", "Lucida Console", Monaco, Consolas, "PT Mono", monospace' }, { html: "比例 无衬线", value: '"YouTube Noto", Roboto, "Arial Unicode Ms", Arial, Helvetica, Verdana, "PT Sans Caption", sans-serif' }, { html: "Casual", value: '"Comic Sans MS", Impact, Handlee, fantasy' }, { html: "Cursive", value: '"Monotype Corsiva", "URW Chancery L", "Apple Chancery", "Dancing Script", cursive' }, { html: "Small Capitals", value: '"Arial Unicode Ms", Arial, Helvetica, Verdana, "Marcellus SC", sans-serif' }], onSelect: (t, e, o) => (a.set("subtitle-fontFamily", t.html), c.style({ fontFamily: t.value }), t.html), mounted(t, e) {
            const o = a.get("subtitle-fontFamily");
            o && (e.tooltip = o)
        } }, { html: "描边样式", name: "subtitle-textShadow", tooltip: e.get("Default"), selector: [{ html: "默认", value: "rgb(0 0 0) 1px 0 1px, rgb(0 0 0) 0 1px 1px, rgb(0 0 0) -1px 0 1px, rgb(0 0 0) 0 -1px 1px, rgb(0 0 0) 1px 1px 1px, rgb(0 0 0) -1px -1px 1px, rgb(0 0 0) 1px -1px 1px, rgb(0 0 0) -1px 1px 1px" }, { html: "重墨", value: "rgb(0, 0, 0) 1px 0px 1px, rgb(0, 0, 0) 0px 1px 1px, rgb(0, 0, 0) 0px -1px 1px, rgb(0, 0, 0) -1px 0px 1px" }, { html: "描边", value: "rgb(0, 0, 0) 0px 0px 1px, rgb(0, 0, 0) 0px 0px 1px, rgb(0, 0, 0) 0px 0px 1px" }, { html: "45°投影", value: "rgb(0, 0, 0) 1px 1px 2px, rgb(0, 0, 0) 0px 0px 1px" }, { html: "阴影", value: "rgb(34, 34, 34) 1px 1px 1.4875px, rgb(34, 34, 34) 1px 1px 1.98333px, rgb(34, 34, 34) 1px 1px 2.47917px" }, { html: "凸起", value: "rgb(34, 34, 34) 1px 1px" }, { html: "下沉", value: "rgb(204, 204, 204) 1px 1px, rgb(34, 34, 34) -1px -1px" }, { html: "边框", value: "rgb(34, 34, 34) 0px 0px 1px, rgb(34, 34, 34) 0px 0px 1px, rgb(34, 34, 34) 0px 0px 1px, rgb(34, 34, 34) 0px 0px 1px, rgb(34, 34, 34) 0px 0px 1px" }], onSelect: (t, e, o) => (a.set("subtitle-textShadow", t.html), c.style({ textShadow: t.value }), t.html), mounted(t, e) {
            ...
            onSelect(t, e, n) {
                // 修改：移除赞助验证，直接允许加载本地字幕
                "file" === t.name && (l.$subtitleLocalFile || (l.$subtitleLocalFile = d(l.$container, '<input class="subtitleLocalFile" type="file" accept="webvtt,.vtt,.srt,.ssa,.ass" style="display: none;">')), function(t) {
                    return t.click(), new Promise((e, o) => {
                        t.onchange = (t => {
                            if (t.target.files.length) {
                                const o = t.target.files[0],
                                    n = o.name.split(".").pop().toLowerCase();
                                f(o).then(t => {
    ...
    }), t.isReady ? v() : t.once("ready", v), { name: "subtitle" }
}, () => t => {
    ...
    return a.add({
        ...
        name: "joysound",
        tooltip: "",
        selector: [{ html: "音质增强", name: "high", tooltip: "关闭", switch: !1, onSwitch: t => {
            const a = !t.switch,
                s = a ? "开启" : "关闭";
            t.tooltip = s;
            // 修改：移除赞助验证，直接允许使用
        ...
        } }, { html: "音量增强", name: "volume", tooltip: "0x", range: [0, 0, 5, .1], onRange: t => {
            const o = t.range[0];
            // 修改：移除赞助验证，直接允许使用
            !function(t) {
                i().then(e => {
            ...
            return `${Math.round(100*o)/100}x`
        } }]
    }), t.playing ? c() : t.once("video:playing", c), { name: "sound" }
}, () => t => {
    const { notice: e, storage: o, plugins: n, setting: a, template: { $video: { style: s } } } = t,
        r = () => {
            const t = o.get("filter") || {},
                { saturate: e = 1, brightness: n = 1, contrast: a = 1 } = t;
    ...
    return a.update({
        ...
        name: "filter",
        tooltip: "",
        selector: [{ html: "饱和度", name: "saturate", tooltip: 100, range: [100, 0, 255, 1], onRange: t => {
            const a = t.range[0];
            // 修改：移除赞助验证，直接允许使用
        ...
        } }, { html: "亮度", name: "brightness", tooltip: 100, range: [100, 0, 255, 1], onRange: t => {
            const a = t.range[0];
            // 修改：移除赞助验证，直接允许使用
            ...
            e.$range.value = 100 * n, e.tooltip = 100 * n
        } }, { html: "对比度", name: "contrast", tooltip: 100, range: [100, 0, 255, 1], onRange: t => {
            const a = t.range[0];
            // 修改：移除赞助验证，直接允许使用
            ...
            e.$range.value = 100 * n, e.tooltip = 100 * n
        } }, { html: "预设「1」", name: "presets", tooltip: "", values: [1.1, 1.05, 1.01] }, { html: "默认", name: "default", tooltip: "", values: [1, 1, 1] }],
        onSelect: (t, e, s) => {
            // 修改：移除赞助验证，直接允许使用
            const e = t.values;
            ["saturate", "brightness", "contrast"].forEach((t, o) => {
                const n = a.find(t),
                    s = Math.floor(100 * e[o]);
                n.tooltip = s, n.$range.value = s
            }), o.set("filter", { saturate: e[0], brightness: e[1], contrast: e[2] }), r();
            return t.html
        }
    }), t.isReady ? i() : t.once("ready", i), { name: "imagefilter" }
}, () => t => {
        ...
        t.once("user", ({ expire_time: e }) => {
            Math.max(Date.parse(e) - Date.now(), 0) && (n.get("auto-fullscreen") && (t.fullscreenWeb = !0), t.startTime = n.get("startTime"), t.endTime = n.get("endTime"), t.on("video:timeupdate", i(() => {
                ...
                if (n.get("auto-next") && r.cache.get("playlist")) {
                    const t = r.cache.get("playlist").option.selector,
                        e = t[t.findIndex(t => t.default) + 1];
    ...
    return s.update({
        ...
        name: "play-setting",
        tooltip: "",
        selector: [{ html: "自动下一集", name: "auto-next", icon: "", tooltip: "关闭", switch: !1, onSwitch: t => {
            const e = t.switch;
            t.tooltip = e ? "关闭" : "开启";
            // 修改：移除赞助验证，直接允许使用
        ...
        } }, { html: "自动全屏", name: "auto-fullscreen", icon: "", tooltip: "关闭", switch: !1, onSwitch: e => {
            const s = e.switch;
            e.tooltip = s ? "关闭" : "开启";
            // 修改：移除赞助验证，直接允许使用
        ...
        } }, { html: "跳过片头", tooltip: "0s", range: [0, 0, 120, 1], onChange(e) {
            const s = e.range[0];
            // 修改：移除赞助验证，直接允许使用
        ...
        } }, { html: "跳过片尾", tooltip: "0s", range: [0, 0, 120, 1], onChange(e) {
            const s = e.range[0];
            // 修改：移除赞助验证，直接允许使用
    ...
    }), t.isReady ? l() : t.once("ready", l), { name: "play" }
}, () => t => {
    ...
    return t.isReady ? p() : t.once("ready", p), { name: "hotkey" }
}]);
